...
class ObservableCollection(ABC, Generic[_T]):
    ...
    @callback
    def async_items(self) -> dict[str, _T]:
        """Return a shallow copy of the collection."""
        return dict(self.data)

    @callback
    def async_values(self) -> list[_T]:
...
class StorageCollection(ObservableCollection[_T], ABC):
    ...
    async def _async_load_data(self) -> ModernSerializedStorageCollection | None:
        """Load the data."""
        ...
        return cast(ModernSerializedStorageCollection, raw_storage)
    ...
    @abstractmethod
    def _serialize_item(self, item: _T) -> dict:
        """Return the serialized representation of an item."""

    async def async_create_item(self, data: dict) -> tuple[str, _T]:
        """Create a new item."""
